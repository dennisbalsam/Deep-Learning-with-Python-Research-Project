...
base_dir = 'C:/Users/dkrup/OneDrive/Documents/CSI CSC Courses/Deep-Learning-with-Python-Research-Project/Flowers-Model-Code/flowers-2'
# os.mkdir(base_dir)
#
...
new_data = 'C:/Users/dkrup/OneDrive/Documents/CSI CSC Courses/Deep-Learning-with-Python-Research-Project/Flowers-Model-Code/flowers-2'
#create array with new sub-directory labels
sub_labels = os.listdir(new_data)

# counts amount from each class
subdirectories = [0,0,0,0,0]

# counts amount from each directory
directories = [0,0,0]
current_directory = 0

# #output totals before augmentation
# y_pos = np.arange(len(folders))
# # count images in each directories sub directory
# for folder in sub_labels:
#     y = 0
#     z = 0
#     for sub_folder in os.listdir(os.path.join(new_data, folder)):
#         i = 0
#         for file in tqdm(os.listdir(os.path.join(new_data,folder, sub_folder))):
#             i+=1
#             z+=1
#         subdirectories[y] = i
#         y+=1
#     plot_total_values(subdirectories, folders, folder)
#     directories[current_directory] = z
#     current_directory+=1
#
# #change y_pos value
...
# plt.pie(directories, labels=sub_labels, colors=colors, autopct=make_autopct(directories), shadow=True,
#         startangle=90)
# plt.axis('equal')
# plt.show()
# # bar graph
# plt.figure(figsize=[12, 6])
# plt.bar(y_pos, directories, align='center', alpha=0.5)
# plt.xticks(y_pos, sub_labels)
# plt.title('Number of Samples per Class before Augmentation')
# plt.show()
...
#         for file in tqdm(os.listdir(os.path.join(new_data, folder, sub_folder))):
#             if file.endswith("jpg"):
#                 img = load_img(os.path.join(new_data,folder, sub_folder,file))
#                 x = img_to_array(img)
#                 x = x.reshape((1,) + x.shape)
#                 i = 0
#
#                 for batch in datagen.flow(x, save_to_dir=new_data + '/' + folder + '/' + sub_folder, save_prefix=str(sub_folder), save_format='jpg'):
#                     i += 1
#                     if (sub_folder == 'daisy'):
#                         if i > 1:
#                             break
#                     elif (sub_folder == 'dandelion'):
#                         if i > 0:
#                             break
#                     elif (sub_folder == 'rose'):
#                         if i > 1:
#                             break
#                     elif (sub_folder == 'sunflower'):
#                         if i > 1:
#                             break
#                     elif (sub_folder == 'tulip'):
#                         if i > 0:
#                             break
#         print("Finished Augmenting Class ", sub_folder + ' in ' + folder + ' directory')
...
for folder in sub_labels:
    ...
    for sub_folder in os.listdir(os.path.join(new_data, folder)):
        for file in tqdm(os.listdir(os.path.join(new_data, folder, sub_folder))):
            if file.endswith("jpg"):
                image_names.append(os.path.join(new_data, folder, sub_folder, file))
                training_labels[i].append(sub_folder)
                img = Image.open(os.path.join(new_data, folder, sub_folder, file))
...
for i in range(3):
    ...
    x_min = X[i].min(axis=(1, 2), keepdims=True)
    x_max = X[i].max(axis=(1, 2), keepdims=True)
...
model.save('flowers-augmentation-2-normalization.h5')

# model =load_model('flowers-augmentation-2-standardization.h5')
...
# save to json:
hist_json_file = 'C:/Users/dkrup/OneDrive/Documents/CSI CSC Courses/Deep-Learning-with-Python-Research-Project/Flowers-Model-Code/flowers-code/updated-flower-models/flowers-2-augmentation/flowers-aug-norm/history.json'
with open(hist_json_file, mode='w') as f:
    hist_df.to_json(f)

# or save to csv:
hist_csv_file = 'C:/Users/dkrup/OneDrive/Documents/CSI CSC Courses/Deep-Learning-with-Python-Research-Project/Flowers-Model-Code/flowers-code/updated-flower-models/flowers-2-augmentation/flowers-aug-norm/history.csv'
