...
def changelr(epoch):
    if epoch == 15:
        ...
        print("changing learning rate to .0005 at epoch 15")
        print("new learning rate = " , K.get_value(model.optimizer.lr))
    if epoch == 30:
        K.set_value(model.optimizer.lr, .0003)
        print("changing learning rate to .0003 at epoch 30")
        print("new learning rate = ", K.get_value(model.optimizer.lr))
...
base_dir = 'C:/Users/dkrup/OneDrive/Documents/CSI CSC Courses/Deep-Learning-with-Python-Research-Project/Flowers-Model-Code/flowers-new-distrub'
# os.mkdir(base_dir)
#
...
#         for file in tqdm(os.listdir(os.path.join(base_dir, folder, sub_folder))):
#             if file.endswith("jpg"):
#                 img = load_img(os.path.join(base_dir,folder, sub_folder,file))
#                 x = img_to_array(img)
#                 x = x.reshape((1,) + x.shape)
#                 i = 0
#
#                 for batch in datagen.flow(x, save_to_dir=base_dir + '/' + folder + '/' + sub_folder, save_prefix=str(sub_folder), save_format='jpg'):
#                     i += 1
#                     if (sub_folder == 'daisy'):
#                         if i > 1:
#                             break
...
# helps us change learning rate during training

from keras.callbacks import LearningRateScheduler
change_lr = LearningRateScheduler(changelr)
...
model.save('flowers-newlr-aug-norm.h5')
...
# save to json:
hist_json_file = 'C:/Users/dkrup/OneDrive/Documents/CSI CSC Courses/Deep-Learning-with-Python-Research-Project/Flowers-Model-Code/flowers-code/updated-flower-models/new-set-distribution/aug-norm-new-lr/history.json'
with open(hist_json_file, mode='w') as f:
    hist_df.to_json(f)

# or save to csv:
hist_csv_file = 'C:/Users/dkrup/OneDrive/Documents/CSI CSC Courses/Deep-Learning-with-Python-Research-Project/Flowers-Model-Code/flowers-code/updated-flower-models/new-set-distribution/aug-norm-new-lr/history.csv'
